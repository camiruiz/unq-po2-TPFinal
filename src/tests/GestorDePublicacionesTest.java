...
class GestorDePublicacionesTest {
	...
	@Test	
	void test017ElGestorDePublicacionesPuedeRealizarBusquedas() {
		...
		listaDeFiltros.add(filtro2);
		
		when(filtro.equals(any())).thenReturn(true);
		when(filtro2.equals(any())).thenReturn(true);
		
		GP.buscarPublicaciones(listaDeFiltros);
		verify(filtro, times(2)).equals(any());
		verify(filtro, times(1)).filtrar(any());
		verify(filtro2, times(1)).filtrar(any());
